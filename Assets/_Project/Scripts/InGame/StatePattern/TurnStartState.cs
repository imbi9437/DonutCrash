...
{
    public class TurnStartState : MonoState
    {
        private const float DonutShotTimeout = 20f;
        ...
        {
            _guidelineRender = guidelineRender;
        ...
        public override void Initialize(MonoStateMachine machine)
        {
            ...
            {
                SetupVisualizers(_battleController.GuidelineRenderer, _battleController.SplineController);
            }
        }

        protected override void OnEnable()
        {
            if(_guidelineRender == null && _battleController != null)
            {
                SetupVisualizers(_battleController.GuidelineRenderer, _battleController.SplineController);
            }
            ...
            {
                Debug.LogError("GuidelineRender를 찾을 수 없습니다.");
            }

            if (_splineController == null)
            {
                Debug.LogError("SplineController를 찾을 수 없습니다.");
            }

            InGameOwner currentTurnOwner = _battleController.GetTurnOwner();
            InGameOwner localOwner = _battleController.GetLocalPlayerOwner();

            string currentNickname = _battleController.GetNickname(currentTurnOwner);
            
            EventHub.Instance?.RaiseEvent(new IGS.ShowTurnStartPopup(currentTurnOwner, currentNickname));

            _isInteractor = (localOwner == currentTurnOwner);

            List<DonutObject> turnDonuts = _battleController.GetDonutsByOwner(_battleController.GetTurnOwner());
            int interactableDonuts = turnDonuts.Count(x => x.Interactable);
            bool resetInteractable = interactableDonuts < 1;
            EventHub.Instance.RaiseEvent(new IGS.TurnStartEvent(_battleController.GetTurnOwner(), resetInteractable));


            EventHub.Instance?.RegisterEvent<IGS.ShotDonutEvent>(OnShotDonutEvent);

            if (_isInteractor)
            {
                Debug.Log($"[TurnStartState] ({localOwner})의 턴. 턴 시작 상태 진입");
                EventHub.Instance?.RegisterEvent<IS.Click>(OnTouch);
                EventHub.Instance?.RegisterEvent<IS.Point>(OnPoint);
            }
            else
            {
                Debug.Log($"[TurnStartState] ({localOwner})가 아닌 {currentTurnOwner}의 턴. 관전자 모드로 턴 시작 상태 진입");
            }

            _cts?.Cancel();
            _cts = new CancellationTokenSource();
            DonutShotTimeoutAsync(DonutShotTimeout, _cts.Token).Forget();

            EventHub.Instance?.RaiseEvent(new AS.PlayUiAudioEvent(AudioType.SFX_Fight_10, 1f));
        }
        ...
        private void OnClickDown(Vector2 position)
        {
            ...
            if (donut.Owner != _battleController.GetTurnOwner())
                return;
        ...
        private void OnClickUp(Vector2 position)
        {
            ...
            if (shotForce.magnitude < 2f)
            {
                EventHub.Instance?.RaiseEvent(new IGS.DragCancelledEvent(
                        _selectedUid, 
                        _battleController.GetTurnOwner()
                    ));
                _selectedUid = null;
                _clickedDonut = null;
                return;
        ...
        {
            Vector3 worldDirection = new Vector3(screenDirection.x, 0f, screenDirection.y).normalized;
            return worldDirection;
        }

        private async UniTask DonutShotTimeoutAsync(float time, CancellationToken ct)
        {
            float timeRemaining = time;
            DeckData deck = _battleController.GetDeckData(_battleController.GetTurnOwner());
            if (deck.fieldDonuts.Count < 1)
            {
                Debug.Log($"{_battleController.GetTurnOwner()}의 필드 도넛 없음. 턴 전환");
                EventHub.Instance?.RaiseEvent(new PS.ShotDonut("", Vector2.zero));
                _battleController.ChangeState(BattleState.TurnRunning);
                return;
            }
            ...
            Debug.Log($"도넛 발사 제한시간 {time}초 초과.");
            EventHub.Instance?.RaiseEvent(new PS.ShotDonut("", Vector2.zero));
            _battleController.ChangeState(BattleState.TurnRunning);
        }

        private void OnShotDonutEvent(IGS.ShotDonutEvent evt)
        {
            _battleController.ChangeState(BattleState.TurnRunning);
        }
    }
